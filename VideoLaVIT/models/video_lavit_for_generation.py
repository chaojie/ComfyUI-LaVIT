...
class VideoLaVITforGeneration(nn.Module):
    ...
    def multimodal_video_generate(self,
        ...
        cpudevice = torch.device('cpu')
        cudadevice = torch.device('cuda')
        self.llama_model.to(cudadevice)
        self.visual_tokenizer.to(cudadevice)
        self.tokenizer_decoder.to(cudadevice)
        ...
        for prompt_str, prompt_type in prompts:
            ...
            if prompt_type == 'image':
                ...
                image_tensor = self.processer(image_input).unsqueeze(0).to(cudadevice)
                with self.maybe_autocast():
            ...
            if prompt_type == 'text':
                ...
                ).to(cudadevice).input_ids[0]
                input_prompts.append(text_tokens)

        input_prompts = [torch.cat(input_prompts, dim=0)]

        self.llama_model.to(cpudevice)
        self.visual_tokenizer.to(cpudevice)
        self.tokenizer_decoder.to(cpudevice)
        torch.cuda.empty_cache()
    ...
    def generate_video(self, prompts, 
        ...
        for turn in range(clip_num):
            ...
            self.visual_tokenizer.to(cudadevice)
            self.tokenizer_decoder.to(cudadevice)
            print(f'turn{turn},ref_image{ref_image}')
            is_tokens = True
            if turn == 0:
            ...
            if ref_image is None:
                # text to video
                print(f'generate_image_tokenids')
                ...
                print(f'generate_motion_tokenids')
    ...
    def video_decoding(self, key_frames, motion_tokens, width, height, cond_on_ref_frame=True, use_linear_guidance=True, 
        ...
        self.tokenizer_decoder.to(cpudevice)
        torch.cuda.empty_cache()
        self.video_decoder.to(cudadevice)
        ...
        self.video_decoder.to(cpudevice)
